...
struct HilData {
  ...
  Eigen::Vector3i
      gps_vel_cm_per_s;      // GPS velocity in cm/s in earth-fixed NED frame
  ...
  uint32_t
      alt_mm;  // Altitude (AMSL, not WGS84), in meters * 1000 (positive for up)
  uint16_t eph_cm;  // GPS HDOP horizontal dilution of position in cm (m*100).
                    // If unknown, set to: 65535
  uint16_t epv_cm;  // GPS VDOP vertical dilution of position in cm (m*100). If
                    // unknown, set to: 65535
  uint16_t vel_1e2m_per_s;  // GPS ground speed (m/s * 100). If unknown, set to:
                            // 65535
  uint16_t cog_1e2deg;      // Course over ground (NOT heading, but direction of
                        // movement) in degrees * 100. If unknown, set to: 65535
  uint16_t
      ind_airspeed_1e2m_per_s;  // Indicated airspeed, expressed as m/s * 100
  uint16_t true_airspeed_1e2m_per_s;  // True airspeed, expressed as m/s * 100*/
  uint8_t fix_type;                   // < 0-1: no fix, 2: 2D fix, 3: 3D fix
  uint8_t satellites_visible;  // Number of satellites visible. If unknown, set
                               // to 255
};
...
class HilListeners {
 public:
  ...
  void AirSpeedCallback(
      const geometry_msgs::TwistStampedConstPtr& air_speed_msg,
      HilData* hil_data) {
  ...
  void PressureCallback(const sensor_msgs::FluidPressureConstPtr& pressure_msg,
    ...
    // computing pressure.
    hil_data->pressure_diff_mBar =
        0.5 * kAirDensity_kg_per_m3 * hil_data->ind_airspeed_1e2m_per_s *

...
// The following boolean constants enable/disable debug printing when certain
// plugin methods are called. Suitable for debugging purposes. Left on
// permanently can swamp std::out and can crash Gazebo.
...
/// \brief    Special-case topic for ROS interface plugin to listen to (if
/// present)
...
/// \param[in]  verbose       If true, gzerror if the parameter is not
/// available.
template <class T>
...
template <typename T>
void model_param(const std::string& world_name,
...
/// \details
///           Short reveiw of discrete time implementation of first order
///           system: Laplace:
///             X(s)/U(s) = 1/(tau*s + 1)
...
class FirstOrderFilter {
 public:
  FirstOrderFilter(double timeConstantUp,
...
/// \brief    Computes a quaternion from the 3-element small angle approximation
/// theta.

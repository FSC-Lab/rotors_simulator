...
void GstCameraPlugin::gstCallback(GstElement* appsrc) {
  ...
  GST_BUFFER_DURATION(frameBuffer) =
      gst_util_uint64_scale_int(1, GST_SECOND, (int)rate);
...
void GstCameraPlugin::startGstThread() {
  ...
  if (!dataSrc || !testSrc || !conv || !encoder || !parser || !payload ||
      !sink) {
  ...
  g_object_set(
      G_OBJECT(encoder), "speed-preset", 2, NULL);  // lower = faster, 6=medium
  ...
  gst_bin_add_many(
      GST_BIN(pipeline), dataSrc, conv, encoder, parser, payload, sink, NULL);

  // Link all elements
  if (gst_element_link_many(
          dataSrc, conv, encoder, parser, payload, sink, NULL) != TRUE) {
  ...
  // Set up appsrc
  g_object_set(
      G_OBJECT(dataSrc), "stream-type", 0, "format", GST_FORMAT_TIME, NULL);
...
void GstCameraPlugin::Load(sensors::SensorPtr sensor, sdf::ElementPtr sdf) {
  ...
  this->newFrameConnection = this->camera->ConnectNewImageFrame(
      boost::bind(&GstCameraPlugin::OnNewFrame,
                  this,
                  _1,
                  this->width,
                  this->height,

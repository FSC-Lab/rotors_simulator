...
/////////////////////////////////////////////////
GimbalControllerPlugin::GimbalControllerPlugin() : status("closed") {
...
void GimbalControllerPlugin::Init() {
  ...
  std::string pitchTopic =
      std::string("~/") + this->model->GetName() + "/gimbal_pitch_cmd";
  this->pitchSub = this->node->Subscribe(
      pitchTopic, &GimbalControllerPlugin::OnPitchStringMsg, this);
  // receive roll command via gz transport
  std::string rollTopic =
      std::string("~/") + this->model->GetName() + "/gimbal_roll_cmd";
  this->rollSub = this->node->Subscribe(
      rollTopic, &GimbalControllerPlugin::OnRollStringMsg, this);
  // receive yaw command via gz transport
  std::string yawTopic =
      std::string("~/") + this->model->GetName() + "/gimbal_yaw_cmd";
  this->yawSub = this->node->Subscribe(
      yawTopic, &GimbalControllerPlugin::OnYawStringMsg, this);
...
ignition::ignition::math::Vector3d GimbalControllerPlugin::ThreeAxisRot(
  ...
  return ignition::ignition::math::Vector3d d(
      atan2(r31, r32), asin(r21), atan2(r11, r12));
}
...
void GimbalControllerPlugin::OnUpdate() {
  if (!this->pitchJoint || !this->rollJoint || !this->yawJoint) return;
  ...
  if (time < this->lastUpdateTime) {
    ...
    double rollError =
        this->ShortestAngularDistance(rollLimited, currentAnglePRYVariable.Y());
    double yawError =
        this->ShortestAngularDistance(yawLimited, currentAnglePRYVariable.Z());
    ...
    if (lowerLimitsPRY.X() < upperLimitsPRY.X()) {
      pitchError = ignition::math::clamp(
          pitchError,
    ...
    // ignition::ignition::math::Vector3d angles =
    // this->imuSensor->Orientation().Euler(); gzerr << "ang[" << angles.X() <<
    // ", " << angles.Y() << ", " << angles.Z()

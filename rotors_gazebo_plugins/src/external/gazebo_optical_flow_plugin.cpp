...
void OpticalFlowPlugin::Load(sensors::SensorPtr _sensor, sdf::ElementPtr _sdf) {
  ...
  if (this->width != 64 || this->height != 64) {
    gzerr << "[gazebo_optical_flow_plugin] Incorrect image size, must by 64 x "
             "64.\n";
  }
  ...
  opticalFlow_pub_ =
      node_handle_->Advertise<opticalFlow_msgs::msgs::opticalFlow>(topicName,
                                                                   10);
  ...
  this->newFrameConnection = this->camera->ConnectNewImageFrame(
      boost::bind(&OpticalFlowPlugin::OnNewFrame,
                  this,
                  _1,
                  this->width,
                  this->height,
...
void OpticalFlowPlugin::OnNewFrame(const unsigned char* _image,
  ...
  int quality = _optical_flow->calcFlow(
      (uint8_t*)_image, frame_time_us, dt_us, flow_x_ang, flow_y_ang);

  if (quality >= 0) {  // calcFlow(...) returns -1 if data should not be
                       // published yet -> output_rate
    // prepare optical flow message
    opticalFlow_message.set_time_usec(
        0);  // will be filled in simulator_mavlink.cpp
    ...
    opticalFlow_message.set_integrated_xgyro(
        0.0);  // get real values in gazebo_mavlink_interface.cpp
    opticalFlow_message.set_integrated_ygyro(
        0.0);  // get real values in gazebo_mavlink_interface.cpp
    opticalFlow_message.set_integrated_zgyro(
        0.0);  // get real values in gazebo_mavlink_interface.cpp
    ...
    opticalFlow_message.set_distance(
        0.0);  // get real values in gazebo_mavlink_interface.cpp
    // send message

...
AttitudeController::AttitudeController() : gravity_(9.81), mass_(1.56779) {}
...
void AttitudeController::ComputeDesiredAngularAcc(
  ...
  R_des = Eigen::AngleAxisd(yaw, Eigen::Vector3d::UnitZ())  // yaw
          * Eigen::AngleAxisd(control_attitude_thrust_reference_(0),
                              Eigen::Vector3d::UnitX())  // roll
          * Eigen::AngleAxisd(control_attitude_thrust_reference_(1),
  ...
  Eigen::Vector3d angular_rate_error =
      angular_rate_ - R_des.transpose() * R * angular_rate_des;

  *angular_acceleration =
      -1 * angle_error.cwiseProduct(gain_attitude_) -
